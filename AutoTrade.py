...
def get_stock_balance(code):
    ...
    if code == 'ALL':
        ...
        stockBalance = pd.DataFrame([[cpBalance.GetHeaderValue(1)], [cpBalance.GetHeaderValue(3)], [cpBalance.GetHeaderValue(4)], [cpBalance.GetHeaderValue(7)]], index=['결제잔고수량', '평가금액', '평가손익', '종목수'], columns = [[cpBalance.GetHeaderValue(0)]])

        stockInfo = pd.Series({'결제잔고수량':str(cpBalance.GetHeaderValue(1)), '평가금액':str(cpBalance.GetHeaderValue(3)), '평가손익':str(cpBalance.GetHeaderValue(4)), '종목수':str(cpBalance.GetHeaderValue(7))})

        #print(stockInfo)

        stockInfo.str.pad(width=10, side='left', fillchar='-')

        print(stockInfo)

        #print(stockBalance)        
        dbgout("\n" + stockInfo.str.pad(width=10, side='right').to_string())
    ...
    for i in range(cpBalance.GetHeaderValue(7)):
        ...
        if code == 'ALL':
            dbgout(str(i+1) + '. ' + stock_name + ' (' + stock_code + ')' 
                + ': ' + str(stock_qty))
...
def get_current_cash():
    ...
    cpCash.SetinputValue(5, 'Y')                # Y: 증거금100, N: 금액조회[default] 미수발생증거금100 퍼센트 징수여부코드
    cpCash.BlockRequest() 
